...
class TestFindMismatch(unittest.TestCase):
    ...
    def test_find_mismatch(self):
        ...
        self.assertEqual(self._strip(finder.find(ref, H)),
...
a_mismatch = """
[Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current_voltage')] | [Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current')] - 'm' attr: [1] | [0]
[Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current_voltage')] | [Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current')] - 'l' attr: [2] | [0]
[Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current_voltage')] | [Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current')] - 't' attr: [-3] | [0]
[Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current_voltage')] | [Dynamics('dyn')>AnalogSendPort('A1')>Dimension('current')] - 'i' attr: [0] | [1]
...
b_mismatch = "[Dynamics('dyn')] - AnalogSendPort keys: ['A1', 'A2'] | ['A1', 'A3']"  # @IgnorePep8
...
c_mismatch = "[Dynamics('dyn')>Alias('A4')] - 'rhs' attr: [C2*SV1] | [2*C2*SV1]"  # @IgnorePep8
...
g_mismatch = "[Dynamics('dyn')>Constant('C1')] - 'value' attr: [-71.0] | [-72.0]"  # @IgnorePep8
...
H = Dynamics(
    ...
    state_variables=[
        StateVariable('SV1', dimension=un.voltage),
        StateVariable('SV2', dimension=un.Dimension('another_current', i=1))],
    regimes=[
        Regime(
            'dSV1/dt = -SV1 / P2',
            'dSV2/dt = A3 / ARP2 + SV2 / P2',
...
I = Dynamics(
    ...
    parameters=[Parameter('P1', dimension=un.voltage),
                ...
                Parameter('P4', dimension=un.current)],
    constants=[Constant('C1', value=-71.0, units=un.mV),
               Constant('C2', value=22.2, units=un.Unit(
                   name="K", dimension=un.temperature, power=0))])

i_mismatch = "[Dynamics('dyn')>Constant('C2')>Unit('degC')] | [Dynamics('dyn')>Constant('C2')>Unit('K')] - 'offset' attr: [273.15] | [0.0]"  # @IgnorePep8
