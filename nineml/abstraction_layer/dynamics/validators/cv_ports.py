    ...
    def __init__(self, component):
        ...
        for ns, output_events in self.output_events.iteritems():
            for output_event in output_events:
                ...
                assert output_event in self.events_ports[
                    ns], "Can't find port definition matching OP-Event: %s" % output_event
            ...
            for ap in analogports:
                if not ap in self.available_symbols[namespace]:
                    raise NineMLRuntimeError(
                        'Unable to find an Alias or State variable for analog-port: %s' % ap)
