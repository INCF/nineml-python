...
def getEquationsExpressionParserFunctions():
    ...
    dictFunctions['__create_constant__'] = daet.Constant
    dictFunctions['sin']   = daet.Sin
    dictFunctions['cos']   = daet.Cos
    dictFunctions['tan']   = daet.Tan
    dictFunctions['asin']  = daet.ASin
    dictFunctions['acos']  = daet.ACos
    dictFunctions['atan']  = daet.ATan
    dictFunctions['sinh']  = daet.Sinh
    dictFunctions['cosh']  = daet.Cosh
    dictFunctions['tanh']  = daet.Tanh
    dictFunctions['asinh'] = daet.ASinh
    dictFunctions['acosh'] = daet.ACosh
    dictFunctions['atanh'] = daet.ATanh
    dictFunctions['log10'] = daet.Log10
    dictFunctions['log']   = daet.Log
    dictFunctions['sqrt']  = daet.Sqrt
    dictFunctions['exp']   = daet.Exp
    dictFunctions['floor'] = daet.Floor
    dictFunctions['ceil']  = daet.Ceil
    dictFunctions['fabs']  = daet.Abs

    # Non-standard functions (multiple arguments)
    dictFunctions['pow']   = daet.Pow
...
class ninemlReduceAnalogPort(object):
    ...
    def addPort(self):
        ...
        port = ninemlAnalogPort(name, daet.eInletPort, self.Model, '')
...
class daetoolsVariableParameterDictionaryWrapper(object):
    ...
    def __init__(self, dictIDs, current_index = None):
        """
        :param dictIDs: python dictionary 'name' : daet.daeVariable/daet.daeParameter; 
    ...
    def __getitem__(self, key):
        """
        Gets the daet.daeVariable object at the given *key* and calls *operator ()*
        with the index equal to the *current_index* argument.
        """
        obj = self.dictIDs[key]
        if isinstance(obj, daet.adouble):
            return obj
        
        elif isinstance(obj, (daet.daeParameter, daet.daeVariable)):
...
class nineml_daetools_bridge(daet.daeModel):
    """
    ...
    def getSpikeInPort(self): 
        for event_port in self.nineml_event_ports:
            if event_port.Type == daet.eInletPort:
                return event_port
    ...
    def getSpikeOutPort(self): 
        for event_port in self.nineml_event_ports:
            if event_port.Type == daet.eOutletPort:
                return event_port
...
def connectEventPorts(portFrom, portTo, parent_model):
    ...
    :raises: RuntimeError
    """
    if (portFrom.Type != daet.eOutletPort) or (portTo.Type != daet.eInletPort):
    ...
    for source_port in source.nineml_analog_ports:
        ...
        for target_port in target.nineml_analog_ports:
            if source_port.Name == target_port.Name:
                if (source_port.Type == daet.eInletPort) and (target_port.Type == daet.eOutletPort):
                    connectPorts(source_port, target_port, parent_model)
                    matching_port_found = True
                
                elif (source_port.Type == daet.eOutletPort) and (target_port.Type == daet.eInletPort):
                    connectPorts(source_port, target_port, parent_model)
                    matching_port_found = True
        ...
        if matching_port_found == False:
            for target_port in target.nineml_reduce_ports:
                if source_port.Name == target_port.Name:
                    ...
                    if (source_port.Type == daet.eOutletPort):
                        connectPorts(source_port, target_port, parent_model)
                        matching_port_found = True
...
class daetools_spike_source(nineml_daetools_bridge):
    ...
    def DeclareEquations(self):
        ...
        for i, t in enumerate(self.spiketimes):
            ...
            self.ON_CONDITION(daet.Time() >= t,  switchTo      = 'State_{0}'.format(i+1),
